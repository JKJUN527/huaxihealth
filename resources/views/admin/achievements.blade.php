...
@section('sidebar')
    @include('components.adminAside', ['title' => 'technology', 'subtitle'=>'achievementsList', 'username' => $data['username']])
@endsection
            ...
            <div class="card">
                    ...
                    <div class="mdl-card__menu">
                        ...
                        <ul class="mdl-menu mdl-menu--bottom-right mdl-js-menu mdl-js-ripple-effect"
                            ...
                            <li class="mdl-menu__item">
                                <a href="/admin/technology/addAchievements">添加科研成果</a>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="body table-responsive">
                    <table class="table table-striped" id="cu-admin-table">
                        ...
                        <tbody>
                        @forelse($data['achievements'] as $achievement)
                            <tr>
                                <td>{{$achievement->id}}</td>
                                <td>{{$achievement->title}}</td>
                                <td>{{mb_substr($achievement->content, 0, 60,'utf-8')}}...</td>
                                <td>{{$achievement->created_at}}</td>
                                <td>
                                    <i class="material-icons detail" data-content="{{$achievement->id}}"
                                       data-toggle='modal' data-target='#detailNewsModal'>visibility</i>
                                    <i class="material-icons delete" data-content="{{$achievement->id}}"
                                       style="margin-left: 16px;">delete</i>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5">暂无成果发布</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
...
@section('custom-script')
    <script type="text/javascript">
        ...
        $(".detail").click(function () {
            ...
            $.ajax({
                url: "/admin/technology/achievementsDetail?nid=" + nid,
                type: "get",
        ...
        $(".delete").click(function () {
            ...
            swal({
                ...
                $.ajax({
                    url: "/admin/technology/achievementsDel?id=" + element.attr('data-content'),
                    type: "get",
                    success: function (data) {
