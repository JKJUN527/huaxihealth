...
@section('sidebar')
    @include('components.adminAside', ['title' => 'technology', 'subtitle'=>'teamList', 'username' => $data['username']])
@endsection
                    ...
                    <div class="mdl-card__menu">
                        ...
                        <ul class="mdl-menu mdl-menu--bottom-right mdl-js-menu mdl-js-ripple-effect"
                            ...
                            <li class="mdl-menu__item">
                                <a href="/admin/technology/addTeam">添加科研团队</a>
                            </li>
                        </ul>
                    </div>
                </div>
                    ...
                    <table class="table table-striped" id="cu-admin-table">
                        ...
                        @forelse($data['team'] as $team)
                            <tr>
                                <td>{{$team->id}}</td>
                                <td>{{$team->title}}</td>
                                <td>{{mb_substr($team->content, 0, 60,'utf-8')}}...</td>
                                <td>
                                    <i class="material-icons detail" data-content="{{$team->id}}"
                                       data-toggle='modal' data-target='#detailNewsModal'>visibility</i>
                                    <i class="material-icons delete" data-content="{{$team->id}}"
                                       style="margin-left: 16px;">delete</i>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4">暂无团队</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
...
@section('custom-script')
    <script type="text/javascript">
        ...
        $(".detail").click(function () {
            ...
            $.ajax({
                url: "/admin/technology/teamDetail?nid=" + nid,
                type: "get",
        ...
        $(".delete").click(function () {
            ...
            swal({
                ...
                $.ajax({
                    url: "/admin/technology/teamDel?id=" + element.attr('data-content'),
                    type: "get",
                    success: function (data) {
